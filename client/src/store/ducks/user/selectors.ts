...
export const selectIsStartedAuth = (state: RootState) =>
  !!selectIsAuthLoading(state) || !!selectIsAuthNever(state);

export const selectUserData = (state: RootState) => selectUser(state).data;
export const selectUserOrders = (state: RootState) => selectUser(state).orders;
